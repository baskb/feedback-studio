...
### Added
- **Comments inside popups, drawers and menus.** The overlay now lives in the browser's top layer (as a manual popover, where the browser supports it), so a site's modal `<dialog>` or popover no longer covers the Point button or makes it unreachable, and the overlay goes back on top each time the page opens another one. Clicks, taps and keys inside the overlay stop at the overlay: a site that closes its menu on any click or key outside it no longer sees the Point button or the composer as "outside", a mouse press on an overlay button leaves the page's focus where it is (menus that close when focus leaves stay open), and Esc closes the composer without also closing the site's dialog. A comment made inside such a container records it as `anchor.layer` (a short selector like `dialog.mnav`), `FEEDBACK.md` prints it as an `inside:` line, the processing skill looks for the element there, and the List shows "in a closed popup" instead of "pin lost" while the popup is shut. Pressing `P` still turns Point mode on without any mouse movement, for menus that close the moment the pointer leaves them and for native `popover` elements, which the browser itself closes on any click outside them. Pins inside a popup are re-resolved when it opens or closes (the page's `open`, `hidden`, `aria-expanded` and `aria-hidden` attributes are watched now, next to node changes).
- **Search and sort in the List.** A search box narrows the List (and the pins on the page) to comments whose text, quoted anchor, retyped wording, author, page, type, status, reply or id contain every word typed. A sort menu orders by latest activity (the default, as before), date created, position on the page, what needs attention first (a lost pin, then an unsure one, then an agent proposal waiting for approval, then an unanswered question, then plain open comments), status, type or most replies, and an arrow flips the direction. The choice is remembered per tab. Pin numbers never change; only the order of the cards does.
- **Bulk actions on what the List shows.** With a filter or a search on, a bar offers "resolve all", "delete all" and "copy as Markdown" for the comments shown. Both destructive ones can be undone in one step.
- **Keyboard navigation.** `j` and `k` move a highlight from pin to pin in page order, `Enter` opens that comment's thread, `r` puts the caret in its reply box, `x` resolves it, `/` jumps to the search box, `l` shows or hides the List, `Ctrl+Z` undoes, and `?` shows the list of shortcuts. None of it fires while you type in a text field.
- **Undo, everywhere.** A status change, a retyped comment, a moved pin, a delete and a bulk action can all be undone, from the toast's Undo button or with `Ctrl+Z`; the last ten are kept. A deleted comment comes back exactly as it was, with its id, thread and screenshots, because the server keeps a deleted comment's files aside for a day.
- **Drag a pin to move it.** Press a pin and drag it (hold it first on a phone) onto another element; the element under the pointer lights up, releasing moves the comment there with the same fresh anchor a re-pin makes, and the move can be undone. Esc cancels.
- **Before and after.** A comment that had a pin-time screenshot gets a second one of the same element once it is resolved and the page has reloaded with the change (only when the element is found with confidence, like every screenshot). The expanded card shows the pair with a slider; the verify and report skills know about the second image.
- **Review rounds.** The List shows the current round and the host can start a new one; every comment made from then on carries its round number, a "This round" chip narrows the List to it, cards from an earlier round show a small tag, and `FEEDBACK.md` groups by round.
- **Share-link commenters can edit and delete their own comments** while those are still open, from the browser they wrote them in. A random token made once per browser is sent with every request; the server stores only its hash and checks it on each edit. Nobody can touch someone else's comment, and the host's rights are unchanged.
- **English and Dutch.** The overlay follows the browser's language (`nl-*` gets Dutch) and an `EN`/`NL` button in the List remembers a choice per browser. A test fails when a sentence shown on screen has no Dutch entry, so the two cannot drift.
- **Version history of a reviewed Markdown file.** The server keeps a copy of the `.md` each time it changes on disk (a live-editing agent, a processing batch, a restore) under `.feedback/history/`, notes which comment the agent was on and which comments were resolved since the previous copy, and tells open overlays. The new **History** button in the List lists the versions with their line counts, shows the diff between any two of them line by line, takes a snapshot on demand, and puts an older version back (the current text is kept as a new version first). When the file changes under you, the overlay says so, offers the diff, and reloads the page once that is safe. An agent marks a batch boundary with `POST /__feedback/api/history/snapshot`; the processing skill does so before and after each batch.
- **The overlay follows a single-page app.** A site with client-side routing changes the address without a page load; the overlay now notices (`pushState`, `replaceState`, back and forward, hash changes), redraws the pins for the new page, files new comments under the new path, and drops a thread or a half-written new comment that belonged to the page you left.
- **Keyboard focus stays where it should.** Tab stays inside an open composer, and inside the List when it covers the whole screen on a phone; closing either puts focus back where it was.
- **`--help` and `--version`.** `feedback-studio --help` prints every flag with a one-line explanation, and `--version` prints the version. Both answer and stop: no server, no browser tab, and they work from any folder. The README table lists the same flags with the same wording, from the same list in the code.
- **A misspelled flag now stops the run.** `--proxi http://localhost:5173` used to be accepted in silence and then ignored, so you got a static folder instead of the dev server you asked for. It now says `unknown option --proxi`, shows the flag list, and exits. Words that are not flags are still fine.
- **`--spa`**, for a built single-page app: a path with no file extension that matches no file gets `index.html` with the overlay on it, instead of a 404. A missing `/app.js` still returns 404, so a broken asset is not hidden. Off unless you pass the flag.
- **`--md-html`**, which renders HTML written inside a Markdown file. See the change below for why it is now a flag rather than the default.

### Fixed
- **A comment whose quoted text appears on several identical elements is no longer treated as a sure find.** Two "Submit" buttons, two bullets with the same sentence: when only the text pointed at the element, the first copy used to count as a confident match, which is exactly the guess the tool promises never to make. Such a pin now shows "pin unsure" until you re-pin it; when the stored position and the text agree on one of the copies, it is still a confident match. The browser self-test (`window.__kbfSelfTest()`) reports how many comments were ambiguous.
- **A pin whose element is gone says "pin lost" again.** The "changed after reply" note from the previous fix also covered a comment whose element could not be found at all; it now applies only to a comment that is still found by position. A "Queued…" reply, or a reply that only proposes alternatives, no longer counts as the agent having applied the change.
- **Re-pinning a comment while Point mode is on no longer opens a new comment box on the same element.** Point mode is paused during the re-pin and comes back afterwards, crosshair cursor included.
- **The agent's "reload the page" request works again after an earlier one was postponed.** When a reload had to wait because you were typing, and you kept typing for over a minute, every later reload request was ignored until you reloaded by hand.
- **A reply draft is kept with the comment it was typed under**, not with whichever comment happened to be expanded last.
- **Tweak Mode previews on the element it read its starting values from**, and only looks the element up again when the page replaced it.
- **Dragging the crop box to the edge of the image no longer closes the crop window.** Releasing the mouse over the dark backdrop used to count as a click on it.
- **The agent is no longer shown as present after you clear the session.** When it says it is leaving, that now holds for a minute: a request that merely looks like the agent, such as an edit hook running after the turn ended, is still written to the activity list but no longer marks it as back. An agent that really is back says so, and appears again at once.
- **The MCP server no longer creates a `.feedback` folder in whatever folder the client happened to start it in.** It writes the processing guide only where a `comments.json` already exists, or where `FEEDBACK_DIR` points, or when a comment is actually added, which is the call that creates the folder anyway.
...
- **Stamping a Markdown file keeps the line endings it already had.** A file written on Windows stayed Windows-style everywhere; before, the whole file was rewritten with the other kind of line ending, so adding one marker made every line of the file look changed.
- **A comment whose text was edited gets its marker updated, not a second one next to it.** Each marker now carries the comment id, so the stamper recognises its own marker on the line and replaces it.
- **The text inside one marker can no longer be mistaken for another comment's quoted text.** Existing markers are ignored while looking for the line to stamp, so a comment can never land on a line just because a neighbouring marker happened to mention its words.
- **The activity hook only talks to a review server on your own computer.** It checks the address in `.feedback/session.json` first and stays quiet if that address is not this machine, so the names of edited files cannot be sent elsewhere. It also stops looking for a session at your project folder (the one holding `.git`) instead of climbing four folders up, so a stray session above your project is never picked up.
- **The feedback skill is allowed to use the task and to-do tools** that its own processing steps ask it to use.

### Changed
- **The anchor rules are now tested on every change.** The code that finds a comment's element again lives in its own module (`lib/anchor.mjs`) that both the browser overlay and the test suite use, and `test/anchor.test.mjs` replays the scenarios from HARNESS.md on a page built for the test: an unchanged page, ordinary edits, shifted siblings with rewritten headings, duplicated elements, sentence selections, ids and test attributes. Every scenario checks the one promise that matters: an anchor is found at the right element, or it says it is unsure. It is never confidently wrong. Until now that was a procedure someone ran by hand in the browser console.
- **The Codex prompt and the AGENTS.md snippet say what the code does.** The prompt for Codex now names all six MCP tools and explains `edits`, `textEdit`, `imageReplace`, variants and picks, `autonomy`, screenshots and spoken comments, which it left out before, so a Codex agent no longer drops the exact CSS values, retyped wording or replacement images a reviewer gave it. `interop/AGENTS.md` is generated from the same text `--seed-agents` writes, and a test fails when either drifts. The MCP smoke test now also covers `get_comment`, `reply` and `set_status`.
- **Muted text and the type labels are easier to read.** Timestamps, quoted anchor text and the small change / improve / expand labels were under the usual contrast minimum for small text; they are darker now, in the light theme only.
- **HTML written inside a Markdown file is shown as text now, not rendered.** A `.md` you were sent is not code you reviewed, and the rendered page can reach the comment list, so a `<script>` in the file is worth refusing by default. It appears on the page as the characters that were typed, and so does a tag written inside a sentence. Fenced code blocks and tables are unchanged. Pass `--md-html` for a document you wrote yourself and want to see in full; that is the behaviour every earlier version had. Links are still checked for script addresses in both modes.
- **Files are sent as a stream, and a browser can ask for part of one.** A video can be scrubbed, a large download can be resumed, and a `HEAD` request gets the headers without the file. Pages are still read whole, because the overlay is added to them before they go out.
- **A redirect from a proxied dev server keeps you on the review page.** When the dev server answered with its own full address, the browser followed it straight off the review server and the overlay was gone. The address now points back through us, for every kind of answer, not only pages.
- **A release checks all three version files against the tag**, naming the one that is off. Before, only `package.json` was compared, so a stale plugin manifest could ship a plugin reporting the wrong version. The version bump also stages exactly those files plus the changelog, instead of everything lying around in the folder.
- **Markers now show the comment id**, as in `<!-- @FB#c_1a2b3c: reword this -->`, so a stamped line points back at the comment it came from. Markers written by an older version are left as they are and are not stamped a second time; if the comment text changed in the meantime, that older marker is rewritten in place.
- **Spoken comments on a Markdown document get Markdown verbs.** "reword this" becomes *rephrase*, "expand this" or "more detail" becomes *expand*, "cut this" becomes *delete*, a question stays a *question*, and anything else is a plain *comment*. Spoken comments on a web page keep fix, change, improve and question.
- **The verify skill can be run twice without redoing everything.** A confirmed comment gets a reply starting with `Verified:`, and a later run skips those unless you ask it to check again with `--again`. A *question* comment is now verified by the presence of the answer in its thread, not by hunting for changed wording that was never meant to change.
- The narration tests now run in CI and before a release, alongside the store and marker tests.
- **The feedback export explains every comment type, not only the three web ones.** The legend at the top of `FEEDBACK.md` now says what to do with a `question` (answer it in a reply with a file and line number, do not edit anything, then resolve) and with the Markdown verbs `comment`, `rephrase`, `expand` and `delete` (edit the source `.md` file, never the rendered page, which is thrown away on the next run).

### Security
- **A link written without quotes could still carry a script.** An address such as `href=javascript:...` written without quotes was never checked, and an SVG `<animate>` element could point a link at such an address after the check had already run. Both are handled now, in a rendered Markdown page and in the alternatives an agent proposes for an element. The check also covers `xlink:href`, `formaction` and `action`, not only `href` and `src`.
- **A shared review link no longer reveals which files were edited.** The activity list still tells everyone on the link what the agent is doing, but the file paths are sent only to you and to an admin link. A view or comment link gets the same lines without them, live and on reload.
...
[Unreleased]: https://github.com/baskb/feedback-studio/compare/v1.2.0...HEAD
