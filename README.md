...
- **Works on your phone** — `--tunnel` opens a real-certificate HTTPS URL (no browser warning, mic works, on *any* network); or stay on your Wi-Fi with `--host 0.0.0.0` + `--https`. By default the server binds to loopback only, so the comment API isn't reachable from the network.
...
| `--tunnel` | Open a public HTTPS URL via a Cloudflare quick tunnel — a **real** cert, so no browser warning, the mic works, and it reaches your phone on any network. Fetches the `cloudflared` helper once; no account needed. |
...
- It matches your pinned snippet to find the right line; if it can't locate one
  confidently, it appends the marker at the **end of the file** instead (the toast
  reports how many landed there).
