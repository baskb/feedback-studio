...
For phone + voice over the LAN add `--https --host 0.0.0.0`; or use `--tunnel`,
which needs no extra flag. (`<FBS>` = the feedback-studio plugin path.)
...
- Locate the target with confidence using the anchor's quoted text (search the
  source), cross-checked with the selector. If you cannot identify it
  confidently, do not edit a guess. Leave it open and say it needs a re-pin.
- Act per the comment `type`: web: `fix` (repair), `change` (apply verbatim),
  ...
  `delete`/`question`. For Markdown, edit the `sourceFile`, not the rendered
  HTML.
- Present changes as a diff for the user. After applying, set the comment's
  status to `resolved` (via `set_status` or by editing comments.json). Honour
  `approved` vs `open`; implement `approved` items.
